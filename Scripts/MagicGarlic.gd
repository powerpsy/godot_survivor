...
func pulsate(tree):
	if is_instance_valid(projectile_reference):
		...
		tween.tween_property(projectile_reference, "scale", Vector2(area + 0.1, area + 0.1), 0.2)
		tween.chain().tween_property(projectile_reference, "scale", Vector2(area, area), 0.2)
